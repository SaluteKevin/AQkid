<template>
    <div class="p-16 min-h-screen bg-gradient-to-b to-pink-200 to-60% from-[#bce1ff] from-10%">
        <h2 class="my-4 text-4xl font-semibold text-gray-600">Make-up Classes</h2>
    ...
    async function fetchRequests(page: number) {
        ...
        const {data: requestsResponse, error: requestsError } = await useApiFetch("api/staff/allMakeUps?page="+page, {});
    ...
    const currentpage = ref(paginate.makeup_page)
    ...
    async function onChangePage(page: any) {
        ...
        await paginate.setMakeUpPage(page.value);
    
        await fetchRequests(paginate.makeup_page);
