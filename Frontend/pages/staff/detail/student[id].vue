            ...
            <div class="grid grid-cols-4 sm:grid-cols-12 gap-6 px-4">
                <div class="col-span-4 sm:col-span-3">
                    <div class="bg-white shadow rounded-lg p-6">
                        <div class="flex flex-col items-center">
                            <img :src="`${config.public.imageBaseURL}${student.profile_image_path}`"
                                class="w-32 h-32 bg-gray-300 rounded-full mb-4 shrink-0">


                            <h1 class="text-xl font-bold">{{ student.first_name }}</h1>
                        ...
                        <div class="mt-6  justify-center">
                            ...
                            <h2 v-if="!student.phone_number">No Phone No.</h2>
                        </div>
                        <hr class="my-6 border-t border-gray-300">
                        <div class="mt-6  justify-center">
                            <p class="text-black font-bold">Email</p>
                            <h2>{{ student.email }}</h2>
                            <h2 v-if="!student.email">No Email</h2>
                        </div>
                ...
                <div class="col-span-4 sm:col-span-9">
                    <div class="bg-white shadow rounded-lg p-6">
                        ...
                        <p class="text-gray-700">Firstname : {{ student.first_name }} </p>
                        <p class="text-gray-700">Middlename : {{ student.middle_name }} </p>
                        <p class="text-gray-700">Lastname : {{ student.last_name }} </p>
                        <p class="text-gray-700">Age : {{ new Date().getFullYear() - new
                            Date(student.birthdate).getFullYear() }} </p>
                        <hr class="my-6 border-t border-gray-300">


                        <h2 class="text-xl font-bold mt-6 mb-4">All Courses</h2>

                        <NuxtLink :to="`/staff/detail/course${course.id}`" v-for="course in student.courses" :key="course.id">
...
const { data: studentResponse, error: studentError } = await useApiFetch(`api/staff/students/${route.params.id}`, {});

if (studentResponse.value) {
    student.value = studentResponse.value;
