<template>

  <div class="h-full static">
    
    <div class="relative" id="home">
      <div class="relative pt-32">
        <div class="text-center mx-auto">
                <h1 class="text-cyan-900  font-bold text-5xl md:text-6xl xl:text-7xl">AquaKids <span class="text-orange-500">Chiangmai</span></h1>
                <p class="mt-8 text-gray-700 text-2xl font-semibold px-32 ">สระว่ายน้ำสำหรับเด็กที่ AquaKids Chiangmai เป็นสถานที่ที่เราเน้นการเรียนรู้ผ่านการเล่นและความสนุก มาร่วมเรียนรู้และสร้างความทรงจำที่น่ารักกับเราวันนี้!</p>
                <div class="mt-16 flex flex-wrap justify-center gap-y-4 gap-x-6">
                  <div class="group relative hover:scale-110 duration-300">
                    <div class="absolute -inset-1 rounded-lg bg-gradient-to-r from-indigo-400 via-cyan-500 to-orange-500 opacity-75 blur transition duration-500 group-hover:opacity-100"></div>
                  <button
                      @click="scrollTo"
                      class="relative flex gap-4 text-white text-xl font-semibold rounded-xl bg-gradient-to-r to-indigo-400 via-cyan-500 from-orange-500 items-center justify-center px-6 py-2
                              "
                    >
                    <svg fill="#ffffff" height="30px" width="30px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 512 512" xml:space="preserve" stroke="#ffffff"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <g> <g> <g> <path d="M297.92,246.72l-4.373-4.8c-2.453-2.56-5.12-5.547-8-8.853c-15.787-17.707-39.573-44.587-74.453-61.333 c-5.333-2.56-11.733-0.32-14.187,5.013c-0.64,1.387-1.067,2.987-1.6,4.587V252.8c-31.36,14.933-58.56,38.08-75.093,58.133 c-18.773-17.173-46.827-36.16-76.373-51.2c-5.227-2.667-11.627-0.64-14.4,4.693c-0.747,1.493-1.173,3.2-1.173,4.907 c0,26.133,10.347,56.213,19.84,78.293C40,364.907,28.693,394.24,28.693,424c0,5.867,4.8,10.667,10.667,10.667 c1.707,0,3.307-0.427,4.907-1.173c29.547-15.04,56.96-33.6,75.947-51.2c16.533,20.053,43.733,43.2,76.16,58.133v60.907 C195.84,507.2,200.64,512,206.507,512c1.6,0,3.2-0.32,4.587-1.067c31.253-14.933,58.24-34.133,80.32-56.96 c73.6-14.4,152.853-83.627,152.853-107.307C444.267,321.707,381.867,273.92,297.92,246.72z M284.693,433.28 c-2.24,0.427-4.373,1.493-5.973,3.2c-17.813,18.773-38.507,34.56-61.12,47.04v-50.24c0-4.267-2.56-8.107-6.507-9.813 c-36.373-15.36-66.667-42.987-80.427-63.573c-1.813-2.667-4.8-4.373-8-4.693h-0.853c-2.987,0-5.76,1.173-7.787,3.413 c-10.987,11.84-32.32,29.333-61.76,46.293c2.773-16.533,8.853-34.88,17.707-52.587c1.387-2.88,1.493-6.187,0.213-9.067 C61.12,322.987,54.933,304,52.053,288c29.867,17.067,51.2,34.453,62.08,46.187c4.053,4.373,10.773,4.587,15.04,0.533 c0.64-0.533,1.173-1.173,1.6-1.813c13.867-20.587,44.053-48.107,80.427-63.573c3.947-1.707,6.507-5.547,6.507-9.813v-60.373 c22.933,14.507,39.68,33.387,52.48,47.787c3.093,3.413,5.867,6.613,8.213,9.173l6.187,6.827c1.28,1.387,2.88,2.453,4.693,2.987 c86.933,27.52,130.773,69.44,134.08,80.213C417.92,360.853,351.787,421.76,284.693,433.28z"></path> <path d="M350.4,320c-8.853,0-16,7.147-16,16s7.147,16,16,16s16-7.147,16-16S359.253,320,350.4,320z M350.4,341.333 c-2.987,0-5.333-2.347-5.333-5.333c0-2.987,2.347-5.333,5.333-5.333s5.333,2.347,5.333,5.333 C355.733,338.987,353.387,341.333,350.4,341.333z"></path> <path d="M441.067,149.333c-23.573,0-42.667,19.093-42.667,42.667c0,23.573,19.093,42.667,42.667,42.667 c23.573,0,42.667-19.093,42.667-42.667C483.733,168.427,464.64,149.333,441.067,149.333z M441.067,213.333 c-11.733,0-21.333-9.6-21.333-21.333s9.6-21.333,21.333-21.333S462.4,180.267,462.4,192S452.907,213.333,441.067,213.333z"></path> <path d="M377.067,128c35.307,0,64-28.693,64-64s-28.693-64-64-64c-35.307,0-64,28.693-64,64 C313.173,99.307,341.76,128,377.067,128z M377.067,21.333c23.573,0,42.667,19.093,42.667,42.667 c0,23.573-19.093,42.667-42.667,42.667c-23.573,0-42.667-19.093-42.667-42.667C334.4,40.427,353.493,21.333,377.067,21.333z"></path> </g> </g> </g> </g></svg>
                        Let's explore
                    <svg fill="#ffffff" height="30px" width="30px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 512 512" xml:space="preserve" stroke="#ffffff"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <g> <g> <g> <path d="M297.92,246.72l-4.373-4.8c-2.453-2.56-5.12-5.547-8-8.853c-15.787-17.707-39.573-44.587-74.453-61.333 c-5.333-2.56-11.733-0.32-14.187,5.013c-0.64,1.387-1.067,2.987-1.6,4.587V252.8c-31.36,14.933-58.56,38.08-75.093,58.133 c-18.773-17.173-46.827-36.16-76.373-51.2c-5.227-2.667-11.627-0.64-14.4,4.693c-0.747,1.493-1.173,3.2-1.173,4.907 c0,26.133,10.347,56.213,19.84,78.293C40,364.907,28.693,394.24,28.693,424c0,5.867,4.8,10.667,10.667,10.667 c1.707,0,3.307-0.427,4.907-1.173c29.547-15.04,56.96-33.6,75.947-51.2c16.533,20.053,43.733,43.2,76.16,58.133v60.907 C195.84,507.2,200.64,512,206.507,512c1.6,0,3.2-0.32,4.587-1.067c31.253-14.933,58.24-34.133,80.32-56.96 c73.6-14.4,152.853-83.627,152.853-107.307C444.267,321.707,381.867,273.92,297.92,246.72z M284.693,433.28 c-2.24,0.427-4.373,1.493-5.973,3.2c-17.813,18.773-38.507,34.56-61.12,47.04v-50.24c0-4.267-2.56-8.107-6.507-9.813 c-36.373-15.36-66.667-42.987-80.427-63.573c-1.813-2.667-4.8-4.373-8-4.693h-0.853c-2.987,0-5.76,1.173-7.787,3.413 c-10.987,11.84-32.32,29.333-61.76,46.293c2.773-16.533,8.853-34.88,17.707-52.587c1.387-2.88,1.493-6.187,0.213-9.067 C61.12,322.987,54.933,304,52.053,288c29.867,17.067,51.2,34.453,62.08,46.187c4.053,4.373,10.773,4.587,15.04,0.533 c0.64-0.533,1.173-1.173,1.6-1.813c13.867-20.587,44.053-48.107,80.427-63.573c3.947-1.707,6.507-5.547,6.507-9.813v-60.373 c22.933,14.507,39.68,33.387,52.48,47.787c3.093,3.413,5.867,6.613,8.213,9.173l6.187,6.827c1.28,1.387,2.88,2.453,4.693,2.987 c86.933,27.52,130.773,69.44,134.08,80.213C417.92,360.853,351.787,421.76,284.693,433.28z"></path> <path d="M350.4,320c-8.853,0-16,7.147-16,16s7.147,16,16,16s16-7.147,16-16S359.253,320,350.4,320z M350.4,341.333 c-2.987,0-5.333-2.347-5.333-5.333c0-2.987,2.347-5.333,5.333-5.333s5.333,2.347,5.333,5.333 C355.733,338.987,353.387,341.333,350.4,341.333z"></path> <path d="M441.067,149.333c-23.573,0-42.667,19.093-42.667,42.667c0,23.573,19.093,42.667,42.667,42.667 c23.573,0,42.667-19.093,42.667-42.667C483.733,168.427,464.64,149.333,441.067,149.333z M441.067,213.333 c-11.733,0-21.333-9.6-21.333-21.333s9.6-21.333,21.333-21.333S462.4,180.267,462.4,192S452.907,213.333,441.067,213.333z"></path> <path d="M377.067,128c35.307,0,64-28.693,64-64s-28.693-64-64-64c-35.307,0-64,28.693-64,64 C313.173,99.307,341.76,128,377.067,128z M377.067,21.333c23.573,0,42.667,19.093,42.667,42.667 c0,23.573-19.093,42.667-42.667,42.667c-23.573,0-42.667-19.093-42.667-42.667C334.4,40.427,353.493,21.333,377.067,21.333z"></path> </g> </g> </g> </g></svg>
                </button>
                  </div>
                  
                </div>
                
        </div>
        <img 
                    class="w-full object-cover"
                    src="https://media.discordapp.net/attachments/1139884248376016906/1161715416142458952/kWxg7GulvATreEgkgASSABJAAEkACSAAJIIE6TYDVEjGVyWRlL6omXqcp4OReiQAa6aEDVEBJAAEkACSAAJIAEkgASQABJAAkig6gn8H5o7yEFNCx1AAAAAElFTkSuQmCC.png?ex=65394eca&is=6526d9ca&hm=9e2e65ff2eee14af569ff2cd2530f07debe8e7c4fa91a2fb78c0f3e671b95520&=&width=1447&height=648">
        <!-- <div class="flex place-content-center justify-center space-x-8 mx-12 absolute inset-x-0 bottom-0">
              <div class="h-full w-full">
                <img 
                  class="object-cover h-72 w-full rounded-full hover:scale-105 duration-300 hover:shadow-xl" 
                  src="https://cdn.discordapp.com/attachments/871313414616657920/1160596548346990632/361185793_738987691571314_899847787237408274_n.png?ex=65353cc3&is=6522c7c3&hm=59e9ba8f8216e9ea2c98517d91d0718f8ed444a1276d21262d552c081b38baf9&"
                  >
              </div>
              <div class="h-full w-full">
                <img 
                  class="object-cover h-72 w-full rounded-full hover:scale-105 duration-300 hover:shadow-xl" 
                  src="https://media.discordapp.net/attachments/871313414616657920/1160594887499071538/362288650_738993328237417_988681206543655468_n.jpg?ex=65353b37&is=6522c637&hm=e1f314f86f5041931f188565534dc6b9e6d8f1b09a97fe6c9012c1d7db2d0132&=&width=818&height=545"
                  >
              </div>
              <div class="h-full w-full">
                <img 
                  class="object-cover h-72 w-full rounded-full hover:scale-105 duration-300 hover:shadow-xl" 
                  src="https://cdn.discordapp.com/attachments/871313414616657920/1160597040829583481/354233673_713421017461315_3218277473228969050_n.png?ex=65353d39&is=6522c839&hm=b23adbca1a8abef24f7735c92454bce3d23e1a92f946566b96d15e7fb942cef9&"
                  >
              </div>
        </div> -->
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">

const emits = defineEmits(['scroll'])

const scrollTo = () => {

  emits('scroll');

}

</script>